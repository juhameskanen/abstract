...
computation with static lookup tables. At every run, the output data
...
the faster the execution completes.

In the end, we optimise the simulation completely. All active
computation has been replaced by precomputed data. What remains are two
...
First, it would be absurd to argue that within a set of identical data
structures, some host active consciousness while others remain inert. If
...
The natural mathematical representative of an equivalence class of
wavefunctions is the density matrix $\rho = |\psi\rangle\langle\psi|$,
which is invariant under global phase rotations. Alice is not any
particular $\psi$. She is $\rho$ --- the class of all descriptions that
...
What the framework says at its face value is that the ultimate laws of
...
projectors that produce the same Alice.
