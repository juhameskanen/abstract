...
Standard quantum mechanics answers: this is how things are, shut-up 
and calculate. This chapter offers a different answer. The quantum
...
This is not a derivation of the Born rule from scratch yet. It is a
motivation: the Born rule is precisely what optimal compression looks
like when a continuous description meets a discrete measurement. 
...
To render the compression hypothesis quantitative,
we require a metric that assigns an informational cost to physical states.

Kolmogorov complexity from Algorithmic Information Theory (AIT) provides the
necessary theoretical foundation. However, Kolmogorov is inherently
uncomputable and non-continuous.

However, lets take a closer look what Kolmogorov complexity says
about complex-valued sinusoidal functions.
Crucially, the cosine function represents the simplest continuous periodic function.
Furthermore, because it is a per-class resource, the intrinsic algorithmic complexity
of any real-world wavefunction is vanishingly small.
The true informational overhead resides not in the foundational function,
but in the specific modes themselves --- not in the code but data.

So we replace Kolmogorov complexity with a continuous, computable alternative.
...
exponentially more bits to describe than smooth ones. They are exponentially
inprobable.
...
but because they compress better and are therefore more probable dominating
the measure.

Smooth predictable data compresses best. 
