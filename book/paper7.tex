...
As we approach the problem from a different angle than QFT, we need a precise vocabulary.
In a compressed universe, two fundamentally different kinds of object exist,
...
what it means to be a pixel. Pauli exclusion is actually a pixel axiom, not a
physical postulate.
...
The standard model is an extraordinarily precise effective description
of particle physics. But it takes bosons as fundamental mediators and derives fermion interactions
from boson exchange. The framework inverts this: fermions are
fundamental, bosons are the story we tell about why fermions correlate.
The story is accurate as an effective description --- QED predictions
match experiment to twelve decimal places --- but it is not
ontologically basic. No detector has ever registered a boson as a
localised pixel. Every detection event is a fermion event.

The open problems of this framework are therefore not about recovering
the standard model's boson ontology from the codec. They are about
explaining fermions. The standard model will then follow as an
effective description of fermion correlations projected into boson
language: accurate, useful, but not fundamental. We are not building a
better standard model. We are explaining why the standard model works.

The three genuine open problems are:

\textbf{Why three generations of charged leptons and not four.}
The $n=2$, $m=1$ fermion class gives charged leptons at successive
complexity levels separated by approximately 4 bits per generation.
The lognormal distribution of Chapter~3 places a finite upper bound on
observable complexity: structures above the lognormal peak are
exponentially suppressed. Making this bound quantitative from the
lognormal parameters gives a derived prediction for the number of
charged lepton generations. This is a calculation the framework already
has the machinery for. It has not yet been carried out.

\textbf{Why quarks have the masses they have.}
The charged lepton mass ladder organises into a near-integer sequence
in $\log_2(m/m_e)$ with 4-bit granularity. The six quarks span several
orders of magnitude and have not been placed on the same ladder. This
is the primary open calculation. If the quarks fit the same 4-bit
granularity structure, the framework makes a precise and falsifiable
prediction about every quark mass in terms of a single unit set by the
electron.

\textbf{Why fermion correlations fall off as $1/r^2$.}
Chapter~9 derived the gravitational $1/r^2$ law from spherical flux
conservation applied to the graviton residual. The identical argument
applied to the $n=2$, $m=1$ antisymmetric residual should give the
electromagnetic $1/r^2$ law. The two derivations differ only in which
fermion complexity class produces the residual. If this works, the
ratio of gravitational to electromagnetic coupling strength --- the
hierarchy problem --- becomes a question about the ratio of two
residual norms, which is a calculation within the framework rather than
an unexplained coincidence of nature.

What the framework does not need to derive separately are the $W$, $Z$,
and Higgs bosons. These are effective descriptions of what happens when
fermion transitions occur near or above the lognormal peak: the
correlations become massive and short-ranged because the underlying
fermion transitions are suppressed by the falling tail of the lognormal
distribution. The Higgs mechanism is not wrong. It is an accurate
effective description of the fermion mass ladder projected into boson
language. The framework does not need to derive the mechanism
separately; it needs to derive the mass ladder, and the Higgs
description follows as a consequence.

Similarly, the antisymmetry of fermionic Fock space --- the question
of whether $\psi(x_1, x_2) = -\psi(x_2, x_1)$ emerges from the boson
matrix structure or must be imposed --- is a many-fermion extension
worth pursuing, but it is secondary. The single-fermion results already
establish the essential structure. Extending to tensor products is a
technical programme, not a conceptual one.

The conceptual programme is simpler and more radical: explain fermions,
and let the standard model take care of itself.
