...
% --- Document Metadata ---
\title{\textbf{Emergent Geodesics from Compression-Maximizing Gaussian Blobs}}
...
\section*{Abstract}
We extend our previous informational theory of quantum gravity, in which observers and particles emerge from compressible informational structures, to a fully \textbf{self-consistent, affine Gaussian blob model}. Each blob is now defined not only by its center, but by its full \textbf{covariance matrix}, allowing arbitrary \textbf{scaling, skewing, and rotation}. Candidate blob configurations are evaluated according to the \textbf{compressibility of their full history}, and the most compressible configurations dominate the probability distribution of observer-realized worlds. In this formulation, high-frequency divergences are naturally suppressed by compressibility, providing an emergent mechanism that prevents singular collapse. Iterative resampling of particle positions and blob updates produces trajectories that approximate \textbf{GR-like geodesics} in the emergent informational metric, establishing a direct link between \textbf{compression-maximizing configurations} and classical gravitational behavior.
