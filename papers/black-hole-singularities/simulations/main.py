...
def parse_args():
    ...
    parser.add_argument("--num_particles", type=int, default=100,
                        help="Number of dust particles in the cloud")
    parser.add_argument("--mass", type=float, default=1.0,
                        help="Black hole mass parameter")
    parser.add_argument("--spin", type=float, default=0.9,
                        help="Black hole spin parameter a")
    parser.add_argument("--r0", type=float, default=6.0,
                        help="Initial radial distance of cloud center")
    parser.add_argument("--spacing", type=float, default=0.02,
                        help="Initial spacing between particles")
    parser.add_argument("--tangential_fraction", type=float, default=0.9,
                        help="Fraction of initial velocity that is tangential")
    parser.add_argument("--radial_fraction", type=float, default=0.1,
                        help="Fraction of initial velocity that is radial")
    parser.add_argument("--dt", type=float, default=1e-3,
                        help="Time step for integration")
    parser.add_argument("--max_t", type=float, default=20.0,
                        help="Maximum simulation time")
    parser.add_argument("--tolerance", type=float, default=1e-6,
                        help="Integration tolerance")
    parser.add_argument("--no_visual", action="store_true",
                        help="Disable visualization")
    parser.add_argument("--animate", action="store_true",
                        help="Render MP4 animation of collapse")
...
def main():
    ...
    if not args.no_visual:
        for sim in results:
            sim.visualize(every_n=2)
            sim.visualize_3d(every_n=4)
            if args.animate:
                safe_label = sim.label.replace(" ", "_").replace("(", "").replace(")", "").replace("=", "")
                outfile = f"{safe_label}.mp4"
                print(f"Generating animation for {sim.label} -> {outfile}")
                sim.animate(save_path=outfile, fps=args.fps)
