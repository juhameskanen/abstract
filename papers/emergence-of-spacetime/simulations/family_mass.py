#!/usr/bin/env python3
import argparse
import matplotlib
matplotlib.use("Agg")
import matplotlib.pyplot as plt
import matplotlib.cm as cm
import numpy as np


def p_analytic(tau):
    return 0.5 * (1.0 - np.exp(-2.0 * tau))


def pattern_curve(k, L, tau):
    p = p_analytic(tau)
    return (p ** k) * ((1.0 - p) ** (L - k))


def classify(k, L):
    if k == 0:
        return "falling"
    elif k >= L / 2:
        return "rising"
    else:
        return "bump"


def peak_tau(k, L):
    ratio = 2.0 * k / L
    if ratio >= 1.0:
        return None
    return -0.5 * np.log(1.0 - ratio)


def family_mass(L, tau_grid):
    """Total probability mass (summed over ALL 2^L patterns, via binomial
    weighting) that a random window's weight k falls in each family, as a
    function of tau. Unlike pattern_curve, this answers 'which family is
    typical', not 'how likely is one specific pattern'."""
    from math import comb
    p = p_analytic(tau_grid)
    mass = {"falling": np.zeros_like(tau_grid),
            "bump":    np.zeros_like(tau_grid),
            "rising":  np.zeros_like(tau_grid)}
    for k in range(L + 1):
        prob = comb(L, k) * (p ** k) * ((1.0 - p) ** (L - k))
        mass[classify(k, L)] += prob
    return mass


def plot_family_mass(L, max_tau, outfile):
    """Plot which family holds the majority of probability mass over time."""
    tau_grid = np.linspace(1e-4, max_tau, 800)
    mass = family_mass(L, tau_grid)

    fig, ax = plt.subplots(figsize=(9, 5.5))
    ax.plot(tau_grid, mass["falling"], color="black", lw=2, label="FALLING (k=0)")
    ax.plot(tau_grid, mass["bump"], color="darkorange", lw=2, label="BUMP (0<k<L/2)")
    ax.plot(tau_grid, mass["rising"], color="steelblue", lw=2, label="RISING (k>=L/2)")
    ax.axhline(0.5, color="gray", lw=0.7, ls=":")
    ax.set_xlabel(r"$\tau$")
    ax.set_ylabel("total probability mass in family")
    ax.set_title(f"Which family is typical? (L={L})\nBUMP dominates almost the entire relaxation")
    ax.legend(loc="center right", fontsize=9.5)
    ax.grid(alpha=0.2)
    fig.tight_layout()
    fig.savefig(outfile, dpi=150)
    print(f"Saved -> {outfile}")

    # report the dominant family at each tau in a coarse sweep
    for tau in [0.01, 0.1, 0.3, 0.5, 1.0, 2.0, 5.0]:
        if tau > max_tau:
            continue
        m = family_mass(L, np.array([tau]))
        vals = {k: v[0] for k, v in m.items()}
        winner = max(vals, key=vals.get)
        print(f"  tau={tau:5.2f}  falling={vals['falling']:.4f}  "
              f"bump={vals['bump']:.4f}  rising={vals['rising']:.4f}  -> {winner}")


def plot_families(L, max_tau, outfile, show_all=True, k_subset=None):
    tau_grid = np.linspace(0.0, max_tau, 600)
    fig, ax = plt.subplots(figsize=(10, 6.5))
    k_values = k_subset if k_subset is not None else range(0, L + 1)
    falling_ks = [k for k in k_values if classify(k, L) == "falling"]
    bump_ks    = [k for k in k_values if classify(k, L) == "bump"]
    rising_ks  = [k for k in k_values if classify(k, L) == "rising"]
    cmap_bump   = cm.get_cmap("autumn_r")
    cmap_rising = cm.get_cmap("winter_r")
    handles = []
    for k in falling_ks:
        curve = pattern_curve(k, L, tau_grid)
        line, = ax.plot(tau_grid, curve, color="black", lw=2.5, zorder=5,
                        label=f"FALLING  k={k}  (only k=0)")
        handles.append(line)
    for k in bump_ks:
        frac = k / (L / 2.0)
        colour = cmap_bump(0.15 + 0.75 * frac)
        curve = pattern_curve(k, L, tau_grid)
        line, = ax.plot(tau_grid, curve, color=colour, lw=1.3, alpha=0.85)
        tp = peak_tau(k, L)
        if tp is not None and tp <= max_tau:
            ax.plot(tp, pattern_curve(k, L, np.array([tp]))[0], "o",
                    color=colour, ms=4, zorder=4)
    for k in rising_ks:
        frac = (k - L / 2.0) / (L / 2.0) if L > 0 else 0
        colour = cmap_rising(0.15 + 0.75 * max(frac, 0))
        curve = pattern_curve(k, L, tau_grid)
        line, = ax.plot(tau_grid, curve, color=colour, lw=1.3, alpha=0.85)
    from matplotlib.lines import Line2D
    legend_elems = [
        Line2D([0], [0], color="black", lw=2.5,
               label=f"FALLING (k=0, {len(falling_ks)} curve)"),
        Line2D([0], [0], color=cmap_bump(0.6), lw=2,
               label=f"BUMP (0<k<L/2, {len(bump_ks)} curves) — peak marked \u25CF"),
        Line2D([0], [0], color=cmap_rising(0.6), lw=2,
               label=f"RISING (k\u2265L/2, {len(rising_ks)} curves)"),
    ]
    ax.legend(handles=legend_elems, loc="upper right", fontsize=9.5, framealpha=0.9)
    ax.set_yscale("log")
    ax.set_xlabel(r"$\tau$ = flip count / n  (normalized bit-flip time)", fontsize=11)
    ax.set_ylabel(r"$P(\mathrm{exact\ pattern}\mid\tau) = p(\tau)^k (1-p(\tau))^{L-k}$   (log scale)",
                  fontsize=11)
    ax.set_title(
        f"Pattern-Probability Shape Families  (L={L})\n"
        r"classified by $k$ vs $L/2$: falling ($k{=}0$), bump ($0{<}k{<}L/2$), rising ($k{\geq}L/2$)",
        fontsize=11)
    ax.axvline(1.0, color="gray", lw=0.8, ls=":", alpha=0.6)
    ax.grid(alpha=0.2, which="both")
    fig.tight_layout()
    fig.savefig(outfile, dpi=150)
    print(f"Saved -> {outfile}")
    print(f"\nL={L}: {len(falling_ks)} falling, {len(bump_ks)} bump, {len(rising_ks)} rising "
          f"(out of {L+1} distinct weight-curves, {2**L} total patterns)")
    print("Sample bump peak times (tau* where curve turns over):")
    for k in bump_ks[:8]:
        tp = peak_tau(k, L)
        print(f"  k={k:3d}  (p*=k/L={k/L:.3f})   tau* = {tp:.3f}")


def main():
    ap = argparse.ArgumentParser()
    ap.add_argument("--L", type=int, default=12, help="pattern length (default 12)")
    ap.add_argument("--max-tau", type=float, default=1.0)
    ap.add_argument("--out", type=str, default="family_curves.png")
    ap.add_argument("--mass-out", type=str, default="family_mass.png",
                     help="output file for the 'which family is typical' plot")
    args = ap.parse_args()
    plot_families(args.L, args.max_tau, args.out)
    print()
    plot_family_mass(args.L, args.max_tau, args.mass_out)


if __name__ == "__main__":
    main()
