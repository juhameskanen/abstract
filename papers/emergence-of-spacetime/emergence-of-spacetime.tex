...
\subsection*{Bitstring Mutation}

In the informational ontology established in Paper I, the universe consists of a static, timeless ensemble containing all $2^n$ possible configurations.
There is no fundamental external dynamics or evolution -- all configurations exist simultaneously and equally.
What we perceive as cosmic expansion, structure formation, or the arrow of time emerges internally as conscious observers navigate or interpret ordered sequences within this fixed configuration space.
Because a computer cannot enumerate the full $2^n$ set, the present work uses random bit-flip mutations merely as a practical sampling method that
traverses configurations in approximate order of increasing execution-trace entropy, starting from the unique zero-entropy (all-zero) state.
The bit-flip process should not be misinterpreted as a proposed physical law, but as a convenient way to generate an entropy-ordered sequence of states for analysis.


\subsection*{Geometric Filters}

The specific geometric rules (distance thresholds, equilateral triangles for atoms, etc.) and even the choice between geometric decoding and recursive 1D pattern matching are secondary.
Extensive testing across more than two hundred different filters, mappings, and hierarchy definitions shows that the qualitative results — zero entropy yields no distinguishable structures, while rising entropy produces hierarchical structures whose abundance statistics are robustly lognormal-like — remain stable.
This robustness indicates that the emergence of complexity is a generic combinatorial feature of increasing informational variety rather than an artifact of any particular decoding scheme.



\section{Conclusions}

The increase of entropy expands the space of distinguishable configurations. When interpreted geometrically, this expansion manifests as increasing spatial separation, while locally stable low-entropy structures remain scale-invariant.
Expanding universe is geometric interpretation of information of increasing entropy.

The simulations demonstrate that structure counts follow predictable distributions, independent of decoding method.
