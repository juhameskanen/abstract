...
to purely fermionic questions: why fermions come in three generations,
why they carry the charges they carry, and why their trajectories
produce the force laws they produce.
\end{abstract}
...
observable fermions are ordered by ascending $C_s$.
...
\begin{tabular}{lrrrl}
\toprule
Fermion & Mass (MeV) & $\log_2(m/m_e)$ & Nearest int. & Notes \\
\midrule
Electron &       $0.511$ &  $0.00$ &  $0$ & generation 1 \\
Muon     &     $105.66$  &  $7.69$ &  $8$ & generation 2 \\
Tau      &    $1776.86$  & $11.76$ & $12$ & generation 3 \\
\bottomrule
\end{tabular}
\caption{Charged lepton masses in $\log_2(m/m_e)$ space.
The near-integer sequence with 4-bit granularity suggests a discrete
complexity ladder.
...
fermion class at successive complexity levels, separated by
$\approx 4$ bits per generation (0, 8, 12 bits relative to the
electron).
...
of Paper~V places a finite upper bound on observable complexity.
No fourth charged lepton has been observed.

The quark mass hierarchy spans a wider range and has not yet been
mapped to the ladder.
This is identified as the primary open calculation.
...
transitions at mass-rung 17 produce correlation patterns that
are massive and short-ranged?''
That is a question about the lognormal probability distribution of
Paper~V: fermion transitions near or above the lognormal peak
are suppressed and short-range.
The answer is already in the framework; it needs to be made
quantitative.

The Higgs mechanism in the Standard Model gives mass to the W and Z
by coupling them to a scalar field.
In the codec framework, the question is simpler: why does the
$n{=}2$ fermion class acquire a mass gap above the electron?
The mass ladder answer is: because each successive generation costs
4 more bits, and the lognormal distribution exponentially suppresses
higher-complexity states.
The Higgs is not needed as a separate mechanism.
...
\begin{enumerate}
  \item \textbf{Why three generations of charged leptons and not four.}
        The mass ladder predicts suppression above complexity rung 12.
        Making this bound quantitative from the lognormal of Paper~V
        is a calculation, not a postulate.

  \item \textbf{Why quarks have the masses they have.}
        The six quarks span several orders of magnitude and have not
        yet been placed on the complexity ladder.
        This is the primary open calculation.
...
spacetime is a complex worldline $\psi_{\text{traj}}(t) = x(t) + iy(t)$,
...
\begin{enumerate}
  ...
  \item \textbf{Why three generations.}
        Quantify the lognormal upper bound on observable fermion
        complexity, giving a derived prediction for the number of
        charged lepton generations.

  \item \textbf{Quark mass hierarchy.}
        Place the six quarks on the complexity ladder and verify
        4-bit granularity.
...
\textbf{Open:}
\begin{enumerate}
\item[10.] Physical identification of the $n{=}4$, $m{=}2$ spin-2
...
and a scalar sector.
The mass hierarchy organises into a near-integer sequence in
$\log_2(m/m_e)$ with 4-bit granularity and predicts no stable
charged fermion beyond the tau.

The open problems reduce to three purely fermionic questions: why
three lepton generations, why the quark masses have the values they
have, and whether the $1/r^2$ electromagnetic force law follows from
the fermion residual structure by the same argument that gives
gravity in Paper~VII.
...
\begin{itemize}
  ...
  \item \texttt{s4\_mass\_spectrum.py} ---
        Lepton mass hierarchy; 4-bit codec unit; upper-bound
        prediction.
