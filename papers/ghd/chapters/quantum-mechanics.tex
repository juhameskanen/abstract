...
Measurement yields a single classical outcome in our experience, though the wavefunction encodes all alternatives.
...
Particle-like events are the classical manifestations we observe as reality.



\section{Ultimate Reality Show}

What is the system we have described?

Consider a digital rendering engine. While a scene may contain continuous color values, the display device has only finite precision. To approximate intermediate values, the renderer employs \textbf{probabilistic dithering}. If the actual value of the computed pixel is 9.75, but the rendering hardware supports only values 9 and 10, then the probability
of getting 9 is 25 percent, and the probability of getting the value 10 is 75 percent. Over many pixels, discrete outputs approximate a continuous tone.

Similarly, the \textbf{Born probabilities} act as a weighted sampling of an underlying amplitude.
...
Consider observers existing entirely within the pixel grid of a highly compressed MPEG movie. To those observers, the behavior of every pixel would appear governed by deterministic,
...
What is the deep nature of quantum mechanics? In particular, why does the universe appear to wave at its micro-scale according to this mysterious, abstract, complex-valued, deterministic entity called the \emph{wavefunction}?  

The answer is deceptively simple: we are observing a compressed structure.  
