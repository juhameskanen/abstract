...
General Relativity predicts singularities, but it fails to describe them. However, since General Relativity is fundamentally a theory of geometry—and solutions to the Einstein equations are themselves geometries—the most essential nature of the singularity must be geometric.

In our model, each dust particle and every point of the spacetime fabric in the simulation can be mapped to a unique sequence of memory bits. Together, these elements form a long, continuous bitstring representing a temporal slice of the spatial geometry. By analyzing the information-theoretic correspondence of the black hole—its execution trace—we can bypass the numerical "crash" and observe the underlying state of the information itself.

Initially, we observe that the bitstring is very random:
...
As the simulation evolves and total entropy increases, the entropy of the bitstring encoding the space shows a monotonic decrease over time. 
...
Geometric space time singularities can be viewed as points of zero entropy rather than merely infinite curvature. 
