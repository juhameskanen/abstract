...
What if a home automation software suddenly gained consciousness and began feeling pain when a heating sensor reported high temperatures? This pain might be agonizing, perhaps even intolerable.
The next command in its code would still be to load a value from a register, multiply it by another, calculate a square root, and—based on the return value—write to a memory address to control a heating valve.

What could the software do about its pain? Could it refuse to execute the next assembly instruction? Could it choose not to proceed? It appears doomed to execute its logic, without the agency to choose otherwise.
