"""
Variational Emergent Spacetime (VEST) Solver - Fixed Autograd
===========================================================
...
based on bit-pattern matching, optimized with unitary constraints and 
a smooth spectral seed.
...
def solve_spacetime(
    ...
    # Smooth Quantum Singularity Seed
    y, x = torch.meshgrid(torch.arange(height, device=device), torch.arange(width, device=device), indexing='ij')
    y_c, x_c = height // 2, width // 2
    sigma = min(width, height) * 0.05
    gaussian_seed = torch.exp(-((x - x_c)**2 + (y - y_c)**2) / (2 * sigma**2))
    
    psi[0, :, :, 0] = gaussian_seed
    psi.requires_grad_()
    ...
    for step in range(iterations):
        ...
        # Assemble raw complex components
        psi_raw_c = torch.complex(psi[..., 0], psi[..., 1])

        # --- FIX: OUT-OF-PLACE UNITARY NORMALIZATION ---
        # Normalize each slice out-of-place and store in a pristine list
        normalized_slices = []
        for t in range(time_steps):
            slice_mass = torch.sum(torch.abs(psi_raw_c[t])**2) + 1e-12
            normalized_slices.append(psi_raw_c[t] / torch.sqrt(slice_mass))
        
        # Reconstruct full tensor smoothly without modifying the computation graph in place
        psi_c = torch.stack(normalized_slices, dim=0)
        ...
        for t in range(time_steps):
            ...
            # Resolution-invariant entropy target trajectory
        ...
        if step % 20 == 0:
            print(f"Step {step:03d} | Total Loss: {loss.item():.4f} | Complexity: {complexity_loss.item():.2f}")

    # Final Pass Out-of-Place Normalization for Exporting
    psi_raw_c = torch.complex(psi[..., 0], psi[..., 1])
    final_slices = []
    for t in range(time_steps):
        slice_mass = torch.sum(torch.abs(psi_raw_c[t])**2) + 1e-12
        final_slices.append(psi_raw_c[t] / torch.sqrt(slice_mass))
    
    psi_final_c = torch.stack(final_slices, dim=0)
    final_psi = torch.stack([psi_final_c.real, psi_final_c.imag], dim=-1)
    
    return final_psi.detach()
...
def main():
    ...
    parser.add_argument("--colormap", type=str, default="jet", choices=["jet", "viridis", "magma", "hot"])
