"""
emergent_smoothness.py
======================
Minimal POC demonstrating the G-ψ-D trinity. Not physics
yet, but something smooth - emergent motion, inertia 
and interference.
...
# ── Updated Observer Class ───────────────────────────────────────────────────

class Observer:
    TRAJ_LEN = 16   

    def __init__(self, pos: np.ndarray, sigma: float, phase: float = 0.0, k_vector: np.ndarray = None):
        self.pos      = np.array(pos, dtype=float)
        self.sigma    = sigma
        self.phase    = phase
        # k_vector determines the direction and frequency of spatial ripples
        self.k_vector = np.array(k_vector, dtype=float) if k_vector is not None else np.zeros(2)
        self.traj: List[complex] = [complex(pos[0], pos[1])]

    def spatial_amplitude(self, points: np.ndarray) -> np.ndarray:
        """
        ψ(r) = exp(-|r-r_i|²/(2σ²)) · exp(i * (φ_i + k · (r - r_i)))
        """
        offsets = points - self.pos
        r2 = np.sum(offsets ** 2, axis=1)
        
        # Spatial phase modulation creating the wavelength/fringes
        spatial_phase = np.dot(offsets, self.k_vector)
        
        return np.exp(-r2 / (2.0 * self.sigma ** 2)) * np.exp(1j * (self.phase + spatial_phase))
...
class EmergentGravitySim:
    ...
    def __init__(
        ...
        n_steps:     int   = 200,
        sigma:       float = 0.12,
        k_mod:       float = 0.0,  # Default to 0.0 preserves original behavior
    ) -> None:
        ...
        for i, a in enumerate(angles):
            ...
            # Point the wavevector inward toward the center (cx, cy)
            center_vec = np.array([cx, cy]) - pos
            norm = np.linalg.norm(center_vec)
            k_vector = (center_vec / norm) * k_mod if norm > 0 else np.zeros(2)
