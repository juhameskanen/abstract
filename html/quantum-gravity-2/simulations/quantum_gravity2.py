...
class WavefunctionGravitySim:
    ...
    def _make_grid_points(self):
        ...
        # Use indexing='ij' to align matrix rows with X and columns with Y
        # or keep default and be careful with reshape.
    ...
    def trajectory_to_bitstring(self, segment: np.ndarray) -> List[int]:
        """Encodes a trajectory segment into a binary representation.

        Used as input to QBitwave to evaluate compressibility.

        Args:
            segment: Sequence of 2D points.

        Returns:
            List of bits.
        """
        # Normalize to [0, 1] then encode as 8-bit integers per coordinate
        bits = []
        for point in segment:
            for coord in point:
                # Clamp coordinate to [0, 1] then convert to 8-bit int
    ...
    def description_length_full(self, blob_index, candidate):
        """Computes the informational action for a candidate blob update.

        The action includes:
          - emergent bit-level complexity,
          - inertial continuity,
          - geometric integrity penalties.

        Args:
            blob_index: Index of the blob.
            candidate: Proposed (mean, covariance, phase, omega).

        Returns:
            Scalar informational cost.
        """
        mean_c, cov_c, _, _ = candidate
        history = self.trajs_mean[blob_index]
        segment = np.array(history[-16:] + [mean_c]) # Use 16-step window for QBitwave
        
        # 1. EMERGENCE COST (Using QBitwave)
        bits = self.trajectory_to_bitstring(segment)
        qb = QBitwave(bitstring=bits, fixed_basis_size=8)
        H_emergent = qb.wave_complexity() 
        
        # 2. INERTIAL COST (Euclidean displacement)
        ...
        L_scale = (np.log(np.linalg.det(cov_c) / self.target_area[blob_index]))**2
        
        # Final Informational Action
        return H_emergent + (10.0 * D) + (15.0 * L_shape) + (50.0 * L_scale)

    def assign_particles_to_blob(self, idx):
        """Assigns nearby particles to a given blob using Mahalanobis distance.

        This method implements the geometric observer filter. Only particles
        sufficiently consistent with the blob’s current geometry are considered
        part of the object. This prevents information leakage into the environment
        or other blobs.
    ...
    def generate_candidates_for_blob(self, idx, assigned):
        ...
        Candidates are derived from the maximum-likelihood estimate of the
        assigned particles, with stochastic perturbations to explore nearby
        geometric configurations. The blob’s total area is conserved to enforce
        geometric identity.
    ...
    def update_stats(self):
        ...
        Computes distance, velocity, and acceleration of each blob relative
        to the system barycenter. These quantities are not used in dynamics;
        they exist purely for analysis and visualization.
        """
    ...
    def run(self, save_gif="quantum-gravity-2.gif", fps=15):
        """Runs the simulation and renders the animation.

        The simulation loop alternates between:
          1. Computing a global probability density from wavefunctions,
          2. Sampling particles from this density,
          3. Updating each blob by selecting the most compressible candidate,
          4. Rendering diagnostics and geometry.

        Args:
            save_gif: Output filename for the animation.
        ...
        def animate(step):
            ...
            for i in range(self.n_blobs):
                ...
                # MDL Selection via QBitwave complexity
            ...
            # Individual Particle Points (The 'Grain') ---
            ...
            for i, wf in enumerate(self.wavefunctions):
                ...
                # Identify the angle of the first eigenvector
                # vecs[:, 0] is the eigenvector for vals[0]
                angle = np.degrees(np.arctan2(vecs[1, 0], vecs[0, 0]))
                
                # Match width/height to their respective eigenvalues
                # vals[0] corresponds to the eigenvector at 'angle'
